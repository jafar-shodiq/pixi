...
drone_insert_func = FunctionDeclaration(
    ...
    description=(
        ...
        "Use project `pixi-proj`, dataset `queen_authenbytik`, table `transaction`."
    ),
    parameters={
        "type": "object",
        "properties": {
...
speech_to_text_html = '''
    <div>
        <button id="start-record-btn" style="font-size: 1.2em; padding: 10px 20px;">🎤 Start Recording</button>
        <input type="text" id="transcript" placeholder="Your spoken text will appear here..." style="width: 100%; padding: 10px; margin-top: 10px; font-size: 1.2em;">
    </div>
    <script>
    ...
    if hasattr(response_part, "function_call") and response_part.function_call is not None:
        try:
            ...
            if response_part.function_call.name == "generate_insert_query":
                cleaned_query = """
                INSERT INTO `pixi-proj.queen_authenbytik.transaction`
                ...
                cleaned_query = """
                UPDATE `pixi-proj.queen_authenbytik.transaction`
...
query = """
SELECT *
FROM `pixi-proj.queen_authenbytik.transaction`
